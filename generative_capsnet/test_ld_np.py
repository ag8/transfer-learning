...
def main(_):
    ...
    # For output data
    f1 = open('out_capsgan_test_on_ld' + str(num_examples) + '_np.csv', 'w+', 0)

    with tf.Session() as sess:
        # Initialize the graph, and receive the queue coordinator and the training monitor
        coord, training_monitor = u.init(sess)

        saver.restore(sess, "/mas/u/mkkr/andrewg/transfer-learning/generative_capsnet/saved_state/model51209.ckpt")
